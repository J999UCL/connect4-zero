"""Benchmark the batched root/action search path."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Benchmark batched root/action search throughput.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    search = BatchedRootActionMCTS(config)
    roots = Connect4x4x4Batch(batch_size=args.batch_size, device=device)
    ...
    for index in range(args.warmup):
        ...
        logger.info(
            "warmup_iteration=%s duration=%s visits=%s policy_mean_sum=%.6f",
            index,
    ...
    for index in range(args.iterations):
        ...
        logger.info(
            "iteration=%s duration=%s roots=%s visits=%s rollout_games_est=%s roots_per_sec=%.2f "
